...
CREATE TABLE `permission` (
  ...
  `permName` varchar(255) DEFAULT NULL COMMENT '权限名称',
  `permCode` varchar(255) DEFAULT NULL COMMENT '权限编码',
  `permType` varchar(10) DEFAULT 'menu' COMMENT '权限类型, menu:菜单, res:资源',
  `parentId` bigint(20) unsigned DEFAULT NULL COMMENT '上级权限ID',
  `url` varchar(255) DEFAULT NULL COMMENT 'URL路径',
  `sequence` smallint(5) unsigned DEFAULT NULL COMMENT '排序',
  PRIMARY KEY (`id`),
  KEY `idx_perm_permName` (`permName`) USING BTREE,
  KEY `idx_perm_permCode` (`permCode`) USING BTREE,
  KEY `idx_perm_parentId` (`parentId`)
...
CREATE TABLE `role` (
  ...
  PRIMARY KEY (`id`),
  KEY `idx_role_roleName` (`roleName`) USING BTREE,
  KEY `idx_role_roleCode` (`roleCode`) USING BTREE
...
CREATE TABLE `role_perm` (
  ...
  `permId` bigint(20) unsigned NOT NULL
...
CREATE TABLE `user` (
  ...
  `sex` tinyint(2) unsigned DEFAULT '3' COMMENT '性别 - 1:男, 2:女, 3:保密',
  PRIMARY KEY (`id`),
