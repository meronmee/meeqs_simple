...
public final class BaseUtils {	
	...
	public static double randomAB(double min, double max){
		if(max > min){ 
			double s = nextDouble(max)%(max-min+1) + min;
			return s;
		}else{
			Double d = Math.floor(Math.random());
			return d;
	...
	public static double nextDouble(Double n) {  
		Random random = new Random();
		double d = random.nextDouble() * random.nextInt(n.intValue());  
		return d;  
	} 
	/**
	 * 生成0~n范围内的随机数
	 ...
	 * @return
	 */
	public static long randomAB(long min, long max) {  
		if(max > min){
			long s = nextLong(max)%(max-min+1) + min;
			return s;
		}else{
			Double d = Math.floor(Math.random());
			return d.longValue();
	...
	*/
     ...
     * @return
     */
    public static String success(String msg, Object data, final SerializeFilter... filters){
        Map<String,Object> map = new HashMap<String,Object>();
        map.put("code", 200);
        map.put("msg", msg);
        map.put("data", data);
    ...
    public static String success(String msg){
        Object data = null;
        return success(msg, data);//null值在JSON.toJSONString时会跳过
    }
    /**
     *
     * @return
     ...
     * @return
     */
    public static String success(String msg, String key, Object value){
        Map<String,Object> data = new HashMap<String,Object>();
        data.put(key, value);
